...
{			
	...
	public static void OpenEditorAsUtility ()
	{
		...
		{
			var window = EditorWindow.CreateInstance<VPaint>();
			window.ShowUtility();
			if(AnimatedVegetationEditor.instance != null)
			{
				window.minSize = AnimatedVegetationEditor.instance.minSize;
				window.position = AnimatedVegetationEditor.instance.position;
			}
			window.titleContent = new GUIContent("动态植物编辑");
		}
	}
...
//		SetupColliders ();
    ...
    public bool clearCacheOfAnimatedVegatationEditor = true;

	public void OnDisable () {
        if (AnimatedVegetationEditor.instance != null)
        {
            if (clearCacheOfAnimatedVegatationEditor)
            {
                AnimatedVegetationEditor.instance.ClearEditGo();
            }
        }
...
//		SetupColliders();
	...
	private void SetAsModalWindow()
	{
		if(AnimatedVegetationEditor.instance != null)
		{
			if(EditorWindow.focusedWindow == AnimatedVegetationEditor.instance)
			{
				this.Focus();
			}

			Rect ownerPos = AnimatedVegetationEditor.instance.position;
			Rect thisPos = this.position;
			if(!Mathf.Approximately(ownerPos.x, thisPos.x) || !Mathf.Approximately(ownerPos.y, thisPos.y))
			{
				ownerPos.x = thisPos.x;
				ownerPos.y = thisPos.y;
				AnimatedVegetationEditor.instance.position = ownerPos;
			}
		}
	}
...
//			GUI.contentColor = new Color(0.4f, 0.4f, 0.4f, 1f);
        ...
        if (AnimatedVegetationEditor.instance != null)
        {
            ...
            {
                AnimatedVegetationEditor.instance.CompleteEdit();
                AnimatedVegetationEditor.instance.RefreshLibrary();
                GUI.color = c;
                return;
            }
            GUI.color = c;

            AnimatedVegetationEditor.instance.SetWaveRange(EditorGUILayout.Slider("动画幅度：", AnimatedVegetationEditor.instance.GetWaveRange(), 0, 5));
            AnimatedVegetationEditor.instance.SetWaveFrequency(EditorGUILayout.Slider("动画频率：", AnimatedVegetationEditor.instance.GetWaveFrequency(), 0, 5));
            AnimatedVegetationEditor.instance.SetWaveOffset(EditorGUILayout.Slider("动画偏移：", AnimatedVegetationEditor.instance.GetWaveOffset(), -5, 5));
            EditorGUILayout.BeginHorizontal();
            {
                AnimatedVegetationEditor.instance.SetCulloff(EditorGUILayout.Slider("边缘裁切：", AnimatedVegetationEditor.instance.GetCulloff(), 0, 1));
                GUILayout.FlexibleSpace();
                if (Mathf.Approximately(AnimatedVegetationEditor.instance.GetCulloff(), 0.0f))
                {
                    AnimatedVegetationEditor.instance.SetAlphaTestEnabled(false);
                }
                else
                {
                    AnimatedVegetationEditor.instance.SetAlphaTestEnabled(true);
                }
                GUILayout.Label("AlphaTest" + (AnimatedVegetationEditor.instance.GetAlphaTestEnabled() ? "开启" : "关闭"), EditorStyles.centeredGreyMiniLabel);
            }
            EditorGUILayout.EndHorizontal();
			AnimatedVegetationEditor.instance.SetIsDoubleSide(EditorGUILayout.Toggle("双面显示:", AnimatedVegetationEditor.instance.GetIsDoubleSide()));
            EditorGUILayout.BeginHorizontal();
            {
                AnimatedVegetationEditor.instance.SetIsGlobalWind(EditorGUILayout.Toggle("全局风向:", AnimatedVegetationEditor.instance.GetIsGlobalWind()));
                GUILayout.FlexibleSpace();
                if (AnimatedVegetationEditor.instance.GetIsGlobalWind())
                {
                    GUILayout.Label("动画在世界坐标系中计算", EditorStyles.centeredGreyMiniLabel);
                }
                else
                {
                    GUILayout.Label("动画在模型坐标系中计算", EditorStyles.centeredGreyMiniLabel);
                }
            }
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.BeginHorizontal();
            {
                AnimatedVegetationEditor.instance.SetIsForceEnabled(EditorGUILayout.Toggle("外力影响:", AnimatedVegetationEditor.instance.GetIsForceEnabled()));
                GUILayout.FlexibleSpace();
                GUILayout.Label("通过Alpha通道控制强弱", EditorStyles.centeredGreyMiniLabel);
            }
            EditorGUILayout.EndHorizontal();
            AnimatedVegetationEditor.instance.SetColor(EditorGUILayout.ColorField("叠加颜色:", AnimatedVegetationEditor.instance.GetColor()));
        }
		...
		mainGroup.title = ()=>{
			...
			else
			{
				...
				previewButton(VertexColorPreviewMode.RGB, "RGB通道");
                previewButton(VertexColorPreviewMode.R, "R通道");
                previewButton(VertexColorPreviewMode.G, "G通道");
                previewButton(VertexColorPreviewMode.B, "B通道");
                previewButton(VertexColorPreviewMode.A, "A通道");
		...
		if(layerStack != null)
		{
			...
			controlsGroup.title = ()=>{GUILayout.Label("笔刷工具");};
            ...
            layersGroup.title = () => { GUILayout.Label("层叠加"); };
	...
	{
		menu.AddItem(new GUIContent(prefix + "新建层"), false, NewLayer);
        // 隐藏不需要的功能
	...
	void LayerMenu (GenericMenu menu, int layerIndex, string prefix = "")
	{
		if(layerStack.layers.Count == 1)
			menu.AddDisabledItem(new GUIContent("删除层"));
        else menu.AddItem(new GUIContent("删除层"), false, RemoveLayer, layerIndex);
		...
		menu.AddItem(new GUIContent("复制层"), false, DuplicateLayer, layerIndex);
		
		menu.AddItem(new GUIContent("上移一层"), false, MoveLayerUp, layerIndex);
		
		menu.AddItem(new GUIContent("下移一层"), false, MoveLayerDown, layerIndex);
...
//				layer.tag = EditorGUILayout.Popup("Tag", layer.tag, VPaintLayerTags.instance.tags.ToArray());
	...
	{	
		...
		VPaintGUIUtility.DrawColumnRow(24, 
		()=>{
			VertexEditorControls.radius = EditorGUILayout.Slider("半径", VertexEditorControls.radius, 0, 30);
		});
		...
		VPaintGUIUtility.DrawColumnRow(24, 
		()=>{
			VertexEditorControls.strength = EditorGUILayout.Slider("强度", VertexEditorControls.strength, 0, 100);
		});
		VPaintGUIUtility.DrawColumnRow(24, 
		()=>{
			VertexEditorControls.falloff = EditorGUILayout.Slider("衰减", VertexEditorControls.falloff, 0, 10);
		});
	}
