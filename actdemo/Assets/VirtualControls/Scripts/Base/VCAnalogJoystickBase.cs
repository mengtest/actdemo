...
public abstract class VCAnalogJoystickBase : VCCollideableObject 
{
    ...
    /// 閿佸畾鎽囨潌
    /// </summary>
    public static bool lockJoystick = false;
	...
	protected void AddInstance()
	{
		...
		{
			vcName += "_copy";
			Debug.LogWarning("Atlimitting to add instance with duplicate VCName!\nVCNames must be unique -- renaming this instance to "
				+ vcName);
		}
    ...
    /// <summary>
    /// 褰撳墠鏄剧ずui鐨勭浉鏈猴紝鐢ㄦ潵鍋氬皠绾挎?娴嬬殑锛岄伩鍏嶆憞鏉嗗拰UI鐨勭偣鍑诲啿绐?
    /// </summary>
    public Camera uiCamera = null;

    /// <summary>
    /// 褰撹Е鐐圭偣鍒版寚瀹氬眰鏃讹紝鏈??瑙︾偣灏嗕細琚?拷鐣ぬ
    /// </summary>
    ...
    private Collider movingPartCollider = null;
	...
	// basePart璐村浘鐨勫?搴ド	
	protected int basePartWidth = 0;
    protected int movePartWidth = 0;

    protected Transform anchor;
    private Vector2 screenForward = new Vector2(0, 1);
    private Vector2 screenRight = new Vector2(1, 0);

    private bool joystickBlock = false;

	protected void Start ()
	{
        Init();

		UITexture texture = basePart.GetComponent<UITexture>();
		if (texture != null) 
		{
			basePartWidth = texture.width / 2;
		}
        UITexture moveTexture = movingPart.GetComponent<UITexture>();
        if (moveTexture != null)
	    {
		    movePartWidth = moveTexture.width / 2;
	    }
	}
    ...
    private bool IsHoverIgnoreLayer(Vector3 screenPixelPosition)
    {
        if (uiCamera == null)
        {
            return false;
        ...
        return IsHoverIgnoreLayerByCamera(uiCamera, screenPixelPosition) || IsHoverIgnoreLayerByCamera(Camera.main, screenPixelPosition);
    }

    private bool IsHoverIgnoreLayerByCamera(Camera camera, Vector3 screenPixelPosition)
    {
        if (camera == null)
        {
            return false;
        ...
        Ray ray = camera.ScreenPointToRay(screenPixelPosition);
        RaycastHit[] hits = Physics.RaycastAll(ray, float.MaxValue, ignoreLayers.value);
        if (hits != null)
        {
            int numHits = hits.Length;
            for (int i = 0; i < numHits; ++i)
            {
                if (hits[i].collider != movingPartCollider)
                {
                    return true;
                }
            }
		...
		// we aren't dragging, let's see if we can see if we can
		{
			...
			for (int i = 0; i < VCTouchController.Instance.touches.Count; i++)
			{
				...
				if (positionAtTouchLocation)
				{
                    ...
                    isHoverIgnoreLayer = IsHoverIgnoreLayer(tw.position);
                    if (isHoverIgnoreLayer)
                    {
                        continue;
                    }
...
// debug touch code
	...
	protected virtual void UpdateDelta()
	{
		...
		if (_deltaPixels.sqrMagnitude > _dragDeltaMagnitudeMaxSq)
		{
            ...
            {
                //褰撶Щ鍔ㄤ綅缃?秴鍑烘憞鏉嗚寖鍥存椂锛屾憞鏉嗚窡鐫绉诲姩
                Vector2 basePartOffset = _deltaPixels.normalized * (_deltaPixels.magnitude - dragDeltaMagnitudeMaxPixels);
                Vector3 vec = new Vector3(_movingPartOrigin.x + basePartOffset.x, _movingPartOrigin.y + basePartOffset.y, basePart.transform.position.z);
                SetPosition(basePart, vec);
                _touchOrigin = _colliderCamera.WorldToViewportPoint(basePart.transform.position);
                _touchOriginScreen = _colliderCamera.WorldToScreenPoint(basePart.transform.position);
                _movingPartOrigin = basePart.transform.localPosition;
            }
