...
{
internal class SnailStandardShaderGUI : ShaderGUI
{
    ...
    MaterialProperty offsetLightDirEnabled = null;
    MaterialProperty offsetLightDir = null;
	...
	{
        offsetLightDirEnabled = FindProperty("_OffsetLightDirEnabled", props);
        offsetLightDir = FindProperty("_OffsetLightDir", props);
    ...
    void OffsetLightDirUI()
    {
        {
            EditorGUI.showMixedValue = this.offsetLightDirEnabled.hasMixedValue;
            bool offsetLightDirEnabled = Mathf.Approximately(this.offsetLightDirEnabled.floatValue, 1.0f);

            EditorGUI.BeginChangeCheck();
            offsetLightDirEnabled = EditorGUILayout.Toggle("OffsetLightDirEnabled", offsetLightDirEnabled);
            if (EditorGUI.EndChangeCheck())
            {
                m_MaterialEditor.RegisterPropertyChangeUndo("Offset Light Dir Enabled");
                this.offsetLightDirEnabled.floatValue = offsetLightDirEnabled ? 1.0f : 0.0f;
            }
        ...
        {
            bool offsetLightDirEnabled = Mathf.Approximately(this.offsetLightDirEnabled.floatValue, 1.0f);
            if (offsetLightDirEnabled)
            {
                EditorGUI.showMixedValue = this.offsetLightDir.hasMixedValue;
                Vector4 offsetLightDir = this.offsetLightDir.vectorValue;

                EditorGUI.BeginChangeCheck();
                offsetLightDir = EditorGUILayout.Vector3Field("OffsetLightDir", offsetLightDir);
                if (EditorGUI.EndChangeCheck())
                {
                    m_MaterialEditor.RegisterPropertyChangeUndo("Offset Light Dir");
                    this.offsetLightDir.vectorValue = offsetLightDir;
                }
	...
	bool ShouldEmissionBeEnabled (Color color)
	{
	...
	void SetMaterialKeywords(Material material, WorkflowMode workflowMode)
	{
        ...
        bool offsetLightDirEnabled = Mathf.Approximately(material.GetFloat("_OffsetLightDirEnabled"), 1.0f);
        SetKeyword(material, "_OFFSET_LIGHT_DIR", offsetLightDirEnabled);
	}
