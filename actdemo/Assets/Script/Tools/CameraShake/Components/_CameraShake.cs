...
namespace ThinksquirrelSoftware.Utilities
{
	...
	public class CameraShake : MonoBehaviour
	{
		...
		public static void Shake(int numberOfShakes , float distance, float speed, float shakeRotationX, float decay)
		{
			instance.DoShake(numberOfShakes ,distance ,speed, shakeRotationX, decay);
		}
		...
		public void DoShake(int numberOfShakes, float distance, float speed, float shakeRotationX, float shakeDecay)
		{
			Vector3 seed = Random.insideUnitSphere;
			
			foreach(Camera cam in cameras)
			{
				StartCoroutine(DoShake_Internal(cam, seed, numberOfShakes, this.shakeAmount, new Vector3(shakeRotationX, 0f, 0f), 
                    distance, speed, shakeDecay, this.guiShakeModifier, this.multiplyByTimeScale, null));
			}
		}
