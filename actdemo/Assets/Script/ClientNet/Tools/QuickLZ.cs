...
namespace Fm_ClientNet
{
    public static class QuickLZ
    {
        ...
        {
            return ((source[1] & 2) == 2) ? 9 : 3;
        }

        public static int sizeDecompressed(byte[] source)
        {
            if (headerLen(source) == 9)
                return source[6] | (source[7] << 8) | (source[8] << 16) | (source[9] << 24);
            else
                return source[3];
        }
        ...
        static byte[] destinationl = new byte[4096];
        public static byte[] decompress(byte[] source)
        {
            int level;
            int size = sizeDecompressed(source) + 1;
            int src = headerLen(source) + 1;
            int dst = 1;
            uint cword_val = 1;
            ...
            {
                Array.Clear(destinationl, 0, destinationl.Length);
            ...
            if ((source[1] & 1) != 1)
            {
                ...
                d2[0] = source[0];
                System.Array.Copy(source, src, d2, 1, size - 1);
                return d2;
            }
