...
Shader "Effect/MAD/Scroll 2 Layers Sine AlphaBlended AlphaChannel" {
Properties {
	_MainTex ("Base layer (RGB)", 2D) = "white" {}
	_MainTexAlphaChannel ("Base layer AlphaChannel", 2D) = "white" {}
	_DetailTex ("2nd layer (RGB)", 2D) = "white" {}
	_DetailTexAlphaChannel ("2nd layer AlphaChannel", 2D) = "white" {}
...
SubShader {
	...
	sampler2D _MainTex;
	sampler2D _MainTexAlphaChannel;
	sampler2D _DetailTex;
	sampler2D _DetailTexAlphaChannel;
...
//		#pragma fragmentoption ARB_precision_hint_fastest		
		fixed4 frag (v2f i) : COLOR
		{
			...
			o.a *= tex2D(_MainTexAlphaChannel, i.uv.xy).r * tex2D(_DetailTexAlphaChannel, i.uv.zw).r;
